...
def findReplaceString(S: str, indexes: List[int], 
                    sources: List[str], targets: List[str]) -> str:
    l = len(indexes)
    Slist = list(S); replacements = []

    for i in range(l):
        replacements.append((indexes[i], list(sources[i]), list(targets[i])))
    
    replacements.sort(key=lambda x: x[0])

    shift = 0
    for i, s, t in replacements:
        lssub = len(s)
        ltsub = len(t)

        index = i + shift

        if (Slist[index:index+lssub] == s):
            Slist[index:index+lssub] = t
            shift += (ltsub - lssub)
