...
examples = [
    {'prices' : [7, 1, 5, 3, 6, 4], 'profit': 7},
    {'prices' : [1, 2, 3, 4, 5],    'profit': 4},
...
for e in examples:
    ...
    for i in range(len(e['prices']) - 1):
        maxProfit += max(e['prices'][i + 1] - e['prices'][i], 0)

    print('Stock Prices: [{}], Expected Profit: {}, Actual Profit: {}'
            .format('-'.join(map(str, e['prices'])), e['profit'], maxProfit))
