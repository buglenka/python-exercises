...
\the packages on the conveyor belt being shipped within D days.
...
Output: 6
Explanation: 
A ship capacity of 6 is the minimum to ship all the packages in 3 days 
like this:
1st day: 3, 2
2nd day: 2, 4
3rd day: 1, 4
...
def shipWithinDays(weights, D):
    lo, hi = max(weights), 2*sum(weights)//D

    while lo < hi:
        limit, days, curr = (lo + hi) // 2, 1, 0
